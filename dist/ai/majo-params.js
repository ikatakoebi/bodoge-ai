...
export function selectCombatStepAction(state, player, playerId, actions) {
...
/** M67追加戦闘アクションを選ぶ */
export function selectExtraCombatAction(actions, state, player) {
...
/** M53: タダで魔導具を獲得 */
export function selectFreeToolAction(state, player, _playerId, actions) {
...
/** M27: 水晶玉アンタップ */
export function selectUntapToolAction(actions, _player) {
 ...
 */
export function mutateParams(params, rate = 0.2) {
    const result = { ...params };
    for (const [key, [min, max]] of Object.entries(PARAM_RANGES)) {
        if (Math.random() < rate) {
            ...
            const newVal = Math.min(max, Math.max(min, result[key] + delta));
            result[key] = newVal;
        }
    }
    return result;
 ...
 */
export function crossoverParams(a, b) {
    const result = {};
 ...
 */
export function createParameterizedStrategy(params) {
    // 聖遺物プールの残量比率を計算（0〜1、1=全部残っている=ゲーム序盤）
    function getRelicPoolRatio(state) {
        // relicDeckがない場合はフォールバック: ラウンド数で推定
        if ('relicDeck' in state && Array.isArray(state.relicDeck)) {
            const total = state.relicDeck.length + (state.discardedRelics?.length ?? 0);
            if (total > 0)
                return state.relicDeck.length / total;
        }
    ...
    return {
        ...
        selectAction(state, playerId) {
            ...
            const preferCombat = hasCombatOption && (!hasPurchaseOption || params.combatBeforePurchase > 0.5);

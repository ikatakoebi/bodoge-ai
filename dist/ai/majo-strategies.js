...
// 進化パラメータ（evolved-params.jsonから取得、generation 336 / fitness 5.96 / 70%共進化）
const EVOLVED_PARAMS = {
    saintVPWeight: 3.242266273240494,
    saintRelicWeight: 0.0928188484499306,
    saintLowHPBonus: 0.2274267058424479,
    toolBuyMaxCount: 3.5893509623526696,
    toolPowerWeight: 1.0817123207529293,
    toolCostWeight: 1.159496980801014,
    manaShopThreshold: 1.4934908285773683,
    witchRoundThreshold: 1.9114964241166215,
    witchMinTools: 0.42967308272514204,
    familiarVPThreshold: 0,
    combatPriority: 10.21149581949731,
    purchasePriority: 12.542181245084182,
    manaPriority: 9.154706293093124,
    saintRelicPoolBonus: 0.3315365341974173,
    saintZeroVPWeight: 0.006196029196717134,
    combatBeforePurchase: 0.7140484267291418,
    purchaseBeforeShop: 0.9039275234936146,
    witchMagicModeWeight: 0.4401961855972057,
    relicAggressiveness: 0.36642510329079936,
    untapRelicCombatThreshold: 1.7835150487950173,
    achievementRelicWeight: 1.6633905477696391,
    achievementToolWeight: 1.2652358674886108,
    violenceMinVP: 0,
    violenceMinMana: 5,
    prayerEarlyRound: 0.9593677017396227,
    sacrificeMinRelics: 5.24557987009228,
    familiarForPurchase: 0.35340518220057454,
    effectManaBonus: 1.876732611618131,
    effectCombatBonus: 1.1516165179713183,
    effectDrawBonus: 0,
    effectUntapBonus: 0.046344206877825,
    effectVPBonus: 0,
    phaseEarlyEnd: 1.5222951465751313,
    phaseLateStart: 4.524450745895829,
    combatPriorityLate: 0.8526763369308734,
    purchasePriorityEarly: 1.2696471586313653,
    manaShopThresholdLate: 5.9182073961291035,
    vpDeficitCombatBoost: 0.8625369839329193,
    vpLeadPurchaseBoost: 0,
    opponentSPAwareness: 0,
};
// ── 共通ヘルパー ──
// 護符の戦闘ボーナスを効果テキストからパースする
function getAmuletCombatBonus(tool) {
    if (tool.type !== '護符')
        return 0;
...
function selectToolsForCombat(player, targetHP) {
    ...
    if (greedyPower >= targetHP) {
        ...
        for (let i = greedy.length - 1; i >= 0; i--) {
            ...
            const smaller = nonAmulets.find((t) => !used.has(t.id) && t.basePower >= deficit && t.basePower < current.basePower);
            if (smaller) {
...
function shouldUseExtraCombatRelic(state, player, currentSaintId) {
...
function findViolenceAction(actions, saintId, allowFamiliar = true) {
    // 新方式: combat_select_saint (violence) - useFamiliar=falseを優先
    const normalCombat = actions.find((a) => a.type === 'combat_select_saint' && a.fieldId === 'violence' && a.saintId === saintId && !a.useFamiliar);
    if (normalCombat)
        return normalCombat;
    // 使い魔版combat_select_saint
    if (allowFamiliar) {
        const familiarCombat = actions.find((a) => a.type === 'combat_select_saint' && a.fieldId === 'violence' && a.saintId === saintId && a.useFamiliar);
        if (familiarCombat)
            return familiarCombat;
    }
    // 旧方式フォールバック
    const normalOld = actions.find((a) => a.type === 'field_action' &&
...
function selectFreeToolAction(state, player, playerId, actions) {
    const freeToolActions = actions.filter((a) => a.type === 'select_free_tool');
    if (freeToolActions.length === 0)
        return null;
    // 魔力が最も高いものを選ぶ
    let best = null;
    for (const act of freeToolActions) {
        if (act.type !== 'select_free_tool')
            continue;
    ...
    if (best) {
        const tool = state.toolSupply.find((t) => t.id === best.action.toolId);
        return {
            action: best.action,
...
// M27水晶玉アンタップアクション
function selectUntapToolAction(actions, player) {
...
function selectToolTurnAction(state, actions, player) {
...
const balanced = {
    ...
    selectAction(state, playerId) {
        ...
        if (cathedralAct)
            return { action: cathedralAct, reasoning: `パスより祈祷（SP+マナ1）` };
...
const aggressive = {
    ...
    selectAction(state, playerId) {
        ...
        if (killableSaints.length > 0 && player.mana >= 5) {
            ...
            if (target) {
                const act = actions.find((a) => a.type === 'combat_select_saint' && a.fieldId === 'sacrifice' && a.saintId === target.id);
                if (act)
                    return { action: act, reasoning: `横暴で${target.name}(★${target.victoryPoints})を強制撃破！` };
            }
        }
...
const economist = {
    ...
    selectAction(state, playerId) {
        ...
        if (disposableRelic) {
            const sacrificeAct = actions.find((a) => (a.type === 'field_action' || a.type === 'use_familiar') &&
                'details' in a && a.details.action === 'prayer' && a.details.relicId === disposableRelic.id);
            if (sacrificeAct)
                return { action: sacrificeAct, reasoning: `生贄: 聖遺物→3マナ（経済型）` };
        }

...
// ========== 出力ユーティリティ ==========
function formatPointCard(value) {
    if (value > 0) {
...
function formatHandCard(value) {
    return chalk.yellow.bold(`[${value}]`);
}
function formatPlayerName(name, isHuman = false) {
...
function printRoundHeader(round, total, pointCard, carryOver) {
    printSeparator('═');
    console.log(chalk.bold(`ラウンド ${chalk.magenta(round)} / ${total}`));
...
function printSelections(result, players, verbose, humanId) {
...
function printRoundResult(result, players) {
...
function printScoreTable(state) {
    printSeparator();
...
function printFinalResults(state) {
...
// ========== auto モード ==========
async function runAutoMode(playerCount, gameCount, verbose, strategyList, logFile) {
    ...
    for (let g = 0; g < gameCount; g++) {
        ...
        const assignedStrategies = strategyList.length >= playerCount
            ...
            : Array.from({ length: playerCount }, (_, i) => strategyList[i] ?? getRandomStrategy().id);
    ...
    if (gameCount > 1) {
        ...
        for (const entry of statsEntries) {
            ...
            console.log(`    ${entry.name.padEnd(18)} ${rateStr}%  ${bar}  (${entry.wins}勝/${entry.games}戦)`);
        }
        printSeparator('═');
    }
    // ログ出力
...
// ========== play モード ==========
async function runPlayMode(playerCount, verbose, strategyList, logFile) {
    printTitle();
    ...
    const aiStrategies = strategyList.length >= aiCount
        ...
        : Array.from({ length: aiCount }, (_, i) => strategyList[i] ?? getRandomStrategy().id);
    const players = [

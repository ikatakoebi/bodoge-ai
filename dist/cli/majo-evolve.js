...
import { getMajoStrategy, majoStrategyIds } from '../ai/majo-strategies.js';
// ── 定数 ──
...
function pickRandomBaselineStrategy() {
...
function saveEvolvedParams(params, generation, fitness, nameSuffix = '') {
    ensureDataDir();
    const data = {
        generation,
        fitness,
...
function saveCardStats(stats, nameSuffix = '') {
    ensureDataDir();
    // 勝率相関を計算して追加
    ...
    fs.writeFileSync(path.join(DATA_DIR, fileName), JSON.stringify(output, null, 2), 'utf-8');
}
// ── 表示ヘルパ�E ──
...
function loadSeedParams(seedName) {
...
function parseLockedKeys(lockArg) {
    if (!lockArg.trim())
        return [];
    const validKeys = new Set(Object.keys(DEFAULT_PARAMS));
    const keys = lockArg
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
...
function applyLockedParams(params, baseParams, lockedKeys) {
...
async function runEvolution(options) {
    const { generations, population: popSize, games: gamesPerEval, verbose, name: nameSuffix, seedName, lock, benchmarkName, benchmarkWeight, benchmarkMarginWeight, } = options;
    ...
    let individuals = [
        ...
        ...Array.from({ length: popSize - 1 }, (_, idx) => ({
            params: applyLockedParams(seedData && idx < Math.ceil((popSize - 1) * 0.75)
                ? mutateParams(baseParams, 0.6)
                : randomizeParams(), baseParams, lockedKeys),
            fitness: 0,
            games: 0,
            wins: 0,
        })),
    ...
    for (let gen = 1; gen <= generations; gen++) {
        ...
        for (let i = 0; i < crossoverCount; i++) {
            ...
            const childParams = applyLockedParams(mutateParams(crossoverParams(parentA.params, parentB.params), MUTATION_RATE * 0.5), baseParams, lockedKeys);
...
function parseFloatArg(name, defaultVal) {
    const eqArg = args.find((a) => a.startsWith(`--${name}=`));
    if (eqArg) {
...
function parseStringArg(name, defaultVal) {
    const eqArg = args.find((a) => a.startsWith(`--${name}=`));
    if (eqArg)

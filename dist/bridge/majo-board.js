...
export class MajoBoardSync {
    ...
    setController(ctrl) {
        ...
        this.client.onMajoAction((index) => {
            if (this.controller) {
    ...
    syncBoard(info) {
        ...
        for (const p of info.players) {
            // 魔導具: タップ状態を渡して横倒し表示
            this.placeCards(p.tools.map((t) => t.id), `p_tools_${p.id}`, true, placedInstanceIds, p.tools.map((t) => t.tapped));
        ...
        for (const stack of Object.values(state.cardStacks)) {
            for (const cid of stack.cardInstanceIds || []) {
                inStack.add(cid);
            }
        }
        for (const inst of Object.values(state.cardInstances)) {
            if (placedInstanceIds.has(inst.instanceId))
                continue;
            if (fieldCardIds.has(inst.instanceId))
                continue;
            if (inStack.has(inst.instanceId))
                continue;
            if (KEEP_DEF_IDS.has(inst.definitionId))
                continue;
    ...
    updatePlayerCounters(info) {
        for (const p of info.players) {
            ...
            if (tapManaId) {
                try {
                    this.client.updateCounter(tapManaId, p.tappedMana);
                }
                catch { /* ignore */ }
            }
    ...
    placeCards(engineIds, areaId, faceUp, placedInstanceIds, tappedFlags) {
    ...
    syncWitchCard(player) {
        ...
        if (!witchArea || witchInstances.length === 0)
            return;
    ...
    syncFamiliarCard(player) {
        const familiarInstances = this.client.getCardsByDefinition('M37');
        const witchArea = this.client.getArea(`p_witch_${player.id}`);
        if (!witchArea || familiarInstances.length === 0)
            return;
        const areaPixelX = witchArea.x * 10;
        const areaPixelW = witchArea.width * 10;
        const areaPixelY = witchArea.y * 10;

...
export class ModernArtBoardSync {
    ...
    sync(info) {
        ...
        if (humanIdx >= 0 && info.myHand) {
            this.placeCards(info.myHand.map(c => c.id), `p_hand_p${humanIdx}`, true, // faceUp
            placed);
        }
        // 2. AIプレイヤーの手札 → 裏向きカード（枚数分）
        for (let i = 0; i < info.players.length; i++) {
            const p = info.players[i];
            if (p.id === info.humanPlayerId)
                continue;
    ...
    // ── Private ──
    placeCards(cardIds, areaId, faceUp, placed) {

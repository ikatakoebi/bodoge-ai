...
export function revealPointCard(state) {
...
export function submitSelection(state, playerId, card) {
    const player = state.players.find((p) => p.config.id === playerId);
    if (!player)
        throw new Error(`プレイヤー ${playerId} が見つかりません`);
    if (!player.hand.includes(card))
        throw new Error(`プレイヤー ${playerId} の手札に ${card} はありません`);
    const updatedPlayers = state.players.map((p) => {
        if (p.config.id !== playerId)
            return p;
        return {
            ...p,
            selectedCard: card,
...
export function resolveRound(state, reasoning) {
    ...
    // プレイヤー状態を更新
    const updatedPlayers = state.players.map((p) => {
        if (p.config.id !== winnerId)
            return { ...p, selectedCard: p.selectedCard };
        return {
            ...p,
...
function findWinnerPositive(selections) {
    if (selections.length === 0)
        return null;
    const maxCard = Math.max(...selections.map((s) => s.card));
    const maxPlayers = selections.filter((s) => s.card === maxCard);
    if (maxPlayers.length === 1)
        return maxPlayers[0].playerId;
    // タイを除外して再帰
    const remaining = selections.filter((s) => s.card !== maxCard);
    return findWinnerPositive(remaining);
}
// タイを除外しながら再帰的に勝者を見つける（マイナスカード用）
function findWinnerNegative(selections) {
    if (selections.length === 0)
        return null;
    const minCard = Math.min(...selections.map((s) => s.card));
    const minPlayers = selections.filter((s) => s.card === minCard);
    if (minPlayers.length === 1)
        return minPlayers[0].playerId;
    // タイを除外して再帰
...
export function inferRemainingHands(state) {
    const usedCards = {};

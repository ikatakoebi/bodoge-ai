...
function nextPlayerIndex(state, from) {
    return (from + 1) % state.players.length;
}
export function isModernArtGameOver(state) {
...
export function getAvailableActions(state, playerId) {
    ...
    if (state.phase === 'play_card') {
        if (state.players[state.currentPlayerIndex].config.id !== playerId)
            return [];
        // 手札から1枚選んでプレイ
    ...
    if (state.phase === 'auction' && state.auctionState) {
        ...
        if (auction.auctionType === 'open') {
            ...
            if (biddingPlayer.config.id !== playerId)
                return [];
            // パス
        ...
        if (auction.auctionType === 'once_around') {
            ...
            if (biddingPlayer.config.id !== playerId)
                return [];
            if (auction.onceAroundBids[playerId] !== undefined)
                return []; // 既に入札済み
            actions.push({ type: 'pass_bid', playerId });
        ...
        if (auction.auctionType === 'sealed') {
            if (playerId === auction.sellerId)
                return [];
            if (auction.sealedBids[playerId] !== undefined)
                return []; // 既に入札済み
            // 密封入札: 金額を入力（0=パス）
    ...
    if (state.phase === 'set_fixed_price' && state.auctionState) {
        if (state.auctionState.sellerId !== playerId)
            return [];
        // 固定価格を設定
    ...
    if (state.phase === 'auction' && state.auctionState?.auctionType === 'fixed_price') {
        ...
        if (askPlayer.config.id !== playerId)
            return [];
...
// ── カードプレイ ──
function executePlayCard(state, playerId, cardId) {
    const playerIdx = state.players.findIndex(p => p.config.id === playerId);
    const player = state.players[playerIdx];
    const cardIdx = player.hand.findIndex(c => c.id === cardId);
    if (cardIdx === -1)
        throw new Error(`カード ${cardId} が手札にない`);
    ...
    const newPhase = auctionType === 'fixed_price' ? 'set_fixed_price' : 'auction';
    return {
        ...state,
        players: updatePlayerAt(state.players, playerIdx, { hand: newHand }),
        playedCardsThisRound: newPlayed,
...
function executePlayDouble(state, playerId, cardId, pairCardId) {
    const playerIdx = state.players.findIndex(p => p.config.id === playerId);
    const player = state.players[playerIdx];
    const card = player.hand.find(c => c.id === cardId);
    const pairCard = player.hand.find(c => c.id === pairCardId);
    if (!card || !pairCard)
        throw new Error('カードが手札にない');
    if (card.artist !== pairCard.artist)
        throw new Error('ダブルオークションは同じ画家のカードが必要');
...
// ── オークション処理 ──
function createAuction(state, card, sellerId, auctionType) {
...
function executeBid(state, playerId, amount) {
    if (!state.auctionState)
        throw new Error('オークション中ではない');
    const auction = { ...state.auctionState };
    const player = getPlayer(state, playerId);
    if (amount > player.money)
        throw new Error('所持金が足りない');
    if (auction.auctionType === 'open') {
        if (amount <= auction.currentBid)
            throw new Error('現在の入札額より高くする必要がある');
    ...
    if (auction.auctionType === 'once_around') {
        if (amount <= (auction.currentBid || 0))
            throw new Error('現在の最高額より高くする必要がある');
...
function executePassBid(state, playerId) {
    if (!state.auctionState)
        throw new Error('オークション中ではない');
    const auction = { ...state.auctionState };
    const player = getPlayer(state, playerId);
    if (auction.auctionType === 'open') {
        ...
        // 残りの入札者チェック
        const activeBidders = state.players.filter(p => p.config.id !== auction.sellerId && !auction.passedPlayerIds.includes(p.config.id));
...
function executeSetFixedPrice(state, playerId, price) {
    if (!state.auctionState)
        throw new Error('オークション中ではない');
    const auction = { ...state.auctionState };
    const player = getPlayer(state, playerId);
...
function executeAcceptFixed(state, playerId) {
    if (!state.auctionState)
        throw new Error('オークション中ではない');
    const auction = { ...state.auctionState };
    const player = getPlayer(state, playerId);
...
function executeDeclineFixed(state, playerId) {
    if (!state.auctionState)
        throw new Error('オークション中ではない');
    const auction = { ...state.auctionState };
    const player = getPlayer(state, playerId);
...
function executeSubmitSealed(state, playerId, amount) {
    if (!state.auctionState)
        throw new Error('オークション中ではない');
    const auction = { ...state.auctionState };
    const player = getPlayer(state, playerId);
...
export function getModernArtFinalScores(state) {
...
// ── ヘルパー ──
function updatePlayerAt(players, index, updates) {
    return players.map((p, i) => i === index ? { ...p, ...updates } : p);
}
function findNextBidder(state, auction) {
...
function checkOnceAroundComplete(state, auction) {
    const nonSellers = state.players.filter(p => p.config.id !== auction.sellerId);
...
export function auctionTypeName(type) {
    switch (type) {
...
export function artistEmoji(artist) {
    switch (artist) {

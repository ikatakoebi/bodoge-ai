...
// 戦闘魔力の計算
export function calculateCombatPower(player, tappedToolIds, useCombatRelics = [], useWitch = false, round = 1, activatedAmuletIds = []) {
    let power = 0;
    // タップした魔導具の魔力合計
...
export function calculateCostReduction(player, toolIdsToTapForDiscount) {
    ...
    for (const r of player.relics) {
        ...
        if (costRedMatch)
            reduction += parseInt(costRedMatch[1], 10);
    }
    return reduction;
}
// ── アクション可能チェック ──
...
function executeCombat(state, playerId, saintId, tappedToolIds, combatRelicIds = [], activatedAmuletIds = []) {
    ...
    for (const toolId of tappedToolIds) {
        ...
        if (tool) {
            ...
            if (relicMatch)
                bonusRelicDraw += parseInt(relicMatch[1], 10);
        }
    }
    // 聖遺物を山札から引く（展示なし）
    const drawnRelics = [];
...
// ── 実績チェック ──
function getAchievementScore(player, achievement, combatPower) {
    const cond = achievement.condition;
    // 「魔力で聖者を撃破」→ 戦闘魔力
    if (cond.includes('魔力') && cond.includes('撃破'))
        return combatPower;
    ...
    // 「聖遺物をNつ以上」
    if (cond.includes('聖遺物'))
        return player.relics.length;
    // 「聖者をNつ以上」（聖遺物より後に判定）
    if (cond.includes('聖者'))
        return player.saints.length;
    // 「魔導具をNつ以上」
    if (cond.includes('魔導具'))
        return player.magicTools.length;
    return 0;
}
function getAchievementThreshold(achievement) {
...
export function calculatePassiveRelicVP(player) {
    ...
    for (const relic of player.relics) {
        ...
        if (saintPairMatch) {
            ...
            if (player.saints.some((s) => s.name === name1 || s.name === name2))
                vp += parseInt(vpStr, 10);
            continue;
        }
        ...
        if (hpMatch) {
            ...
            if (player.saints.some((s) => s.hp >= requiredHP))
                vp += bonus;
            continue;
        }
        // 「護符を所持していたら勝利点＋N」

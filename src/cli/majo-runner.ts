...
import {
  ...
  executeAction, getMajoFinalScores, getAvailableActions, getPlayer,
  getEffectiveMagicPower, calculatePassiveRelicVP,
} from '../engine/majo.js';
...
import type { MajoGameState, MajoPlayerState } from '../engine/majo-types.js';
...
function showPlayerStatus(player: MajoPlayerState) {
...
function showFieldStatus(state: MajoGameState) {
  ...
  // 聖者展示
  const saints = state.saintSupply.map((s) =>
    `${s.name}(HP${s.hp}/★${s.victoryPoints})`
  ).join('  ');
...
function describeAction(action: any, state: MajoGameState, player: MajoPlayerState): string {
  switch (action.type) {
    case 'pass':
      return 'パス';
    ...
    case 'use_familiar': {
      ...
      switch (details.action) {
        case 'research': {
          const tool = state.toolSupply.find((t: any) => t.id === details.toolId);
          if (tool) {
        ...
        case 'violence': {
          const saint = state.saintSupply.find((s: any) => s.id === details.saintId);
          const toolNames = details.tappedToolIds.map((id: string) => {
            const t = player.magicTools.find((tool: any) => tool.id === id);
        ...
        case 'prayer': {
          const relic = player.relics.find((r: any) => r.id === details.relicId);
    ...
    case 'use_relic': {
      const relic = player.relics.find((r: any) => r.id === action.relicId);
      return `聖遺物使用 → ${relic?.effect || action.relicId}`;
    }

    case 'extra_combat': {
      const saint = state.saintSupply.find((s: any) => s.id === action.saintId);
      const toolNames = action.tappedToolIds.map((id: string) => {
        const t = player.magicTools.find((tool: any) => tool.id === id);
        return t ? `${t.name}(${getEffectiveMagicPower(t, player.magicTools)})` : id;
      }).join('+');
      if (saint) {
...
async function runAutoGame(verbose: boolean = true) {
...
async function runStats(games: number) {
  console.log(chalk.bold(`\n═══ 魔女ゲー 統計 (${games}ゲーム) ═══\n`));

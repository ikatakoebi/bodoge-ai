...
import type { MajoToolCard, MajoSaintCard, MajoRelicCard, MajoAchievementCard } from './majo-types.js';

const SHEET_ID = '1h7iwwlbE6_QBd3ClFFAW-PrgVx0quFWJgSb0MKrkbfc';
const SHEET_CSV_URL = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/export?format=csv&gid=0`;

export interface MajoCardData {
  tools: MajoToolCard[];
...
let cachedCards: MajoCardData | null = null;

/** スプレッドシートからカードデータを読み込む */
export async function loadCardsFromSheet(): Promise<MajoCardData> {
...
function parseCardsCsv(csv: string): MajoCardData {
  ...
  const tools: MajoToolCard[] = [];
  const saints: MajoSaintCard[] = [];
  const relics: MajoRelicCard[] = [];
  const achievements: MajoAchievementCard[] = [];

  for (let i = 1; i < lines.length; i++) {
    const fields = parseCsvLine(lines[i]);
    let id = fields[col('id')]?.trim();
    if (!id || id.startsWith('#')) continue; // コメント行・トークン行はスキップ
    ...
    if (type === '魔導具') {
      const toolType = name as MajoToolCard['type'];
      tools.push({
      ...
      let timing: MajoRelicCard['timing'] = 'passive';
      if (effect.startsWith('戦闘')) timing = 'combat';
      else if (effect.startsWith('手番')) timing = 'turn';
      // 売り場〜 も手番用
      else if (effect.startsWith('売り場')) timing = 'turn';

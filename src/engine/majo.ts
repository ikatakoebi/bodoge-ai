...
export async function createMajoGame(players: PlayerConfig[]): Promise<MajoGameState> {
  ...
  try {
    ...
    console.warn('スプレッドシート読み込み失敗、ハードコードデータを使用:', (e as Error).message);
  }
...
export function executeAction(state: MajoGameState, action: MajoAction): MajoGameState {
  ...
  switch (action.type) {
    ...
    case 'select_free_tool': {
      ...
      if (tool.cost > 3) throw new Error(`魔導具 ${action.toolId} はコスト${tool.cost}で3コスト以下ではない`);
...
function getAchievementScore(player: MajoPlayerState, achievementId: string, combatPower: number): number {
...
function getAchievementThreshold(achievementId: string): number {
  switch (achievementId) {

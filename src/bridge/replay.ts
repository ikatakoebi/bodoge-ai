import type { ReplayLog, RoundResult, FinalScore } from '../engine/types.js';
import type { BridgeClient } from './client.js';
import type { StepController } from './step-controller.js';
import type { RemoteGameState } from './types.js';

export interface ReplayOptions {
  /** Delay in ms between each major action (card reveal, selections, resolution). Default: 2000 */
  delay?: number;
  /** Delay in ms between individual card movements. Default: 400 */
  cardDelay?: number;
  /** Player slot → area prefix mapping, e.g. playerSlots[0] = 'p0' */
  playerSlots?: string[];
  /** Optional step controller for pause/resume/step keyboard controls */
...
export class ReplayController {
  ...
  private opts: Required<Omit<ReplayOptions, 'stepController'>> & { stepController?: StepController };
  private stepCtrl?: StepController;
  ...
  private stateSnapshots: RemoteGameState[] = [];

  constructor(log: ReplayLog, client: BridgeClient, opts: ReplayOptions = {}) {
    this.log = log;
    this.client = client;
    this.opts = {
   ...
   */
  private isStepMode(): boolean {
    if (!this.stepCtrl) return false;
    return (this.stepCtrl as any).isPaused === true;
  }
   ...
   */
  private async waitOrDelay(ms: number): Promise<boolean> {
    if (this.stepCtrl) {
   ...
   */
  private async quickDelay(ms: number): Promise<void> {
    if (this.isStepMode()) return; // Skip small delays in step mode
    if (this.stepCtrl) {
  ...
  private saveSnapshot(): void {
    const snapshot = JSON.parse(JSON.stringify(this.client.getState())) as RemoteGameState;
  ...
  async stepBack(): Promise<boolean> {
    if (this.stateSnapshots.length <= 1) return false;
  ...
  private async replayRound(round: RoundResult, roundCounterId: string | null, scoreCounters: Map<string, string>): Promise<boolean> {
    ...
    this.saveSnapshot();
    if (!await this.waitOrDelay(this.opts.delay)) return false;
    ...
    if (round.winnerId) {
      ...
      if (winnerSlot) {
        ...
        if (this.stepCtrl && typeof (this.stepCtrl as any).setScores === 'function') {
          (this.stepCtrl as any).setScores(Object.fromEntries(this.playerScores));
        }
        ...
        if (this.stepCtrl && typeof (this.stepCtrl as any).setRoundResult === 'function') {
          (this.stepCtrl as any).setRoundResult(roundResultMsg);
        }
      }
    } else {
      ...
      if (this.stepCtrl && typeof (this.stepCtrl as any).setRoundResult === 'function') {
        (this.stepCtrl as any).setRoundResult(drawMsg);
      }
  ...
  private async showFinalScores(finalScores: FinalScore[]): Promise<void> {

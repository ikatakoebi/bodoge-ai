...
import {
  createModernArtGame, getCurrentPlayer, executeAction,
...
import type {
  ...
  ModernArtGameInfo, ModernArtPlayerInfo, ModernArtFinalScore,
  ModernArtActionChoice, ArtistName,
} from '../engine/modern-art-types.js';
import { ARTIST_NAMES } from '../engine/modern-art-types.js';

// ── 公開インターフェース ──

export interface ModernArtPlayOptions {
...
export class ModernArtPlayController {
  private state: ModernArtGameState;
  private strategies: Map<string, ModernArtAIStrategy> = new Map();
  ...
  private log: string[] = [];
  private resolveAction: ((value: ModernArtAction) => void) | null = null;
  private waitingForHuman = false;
  private waitingPlayerId: string = '';
  private aborted = false;
  private finished = false;
  private finalScores: ModernArtFinalScore[] | null = null;

  private onUpdate: (() => void) | null = null;
  private _strategyNames: string[] = [];

  constructor(opts: ModernArtPlayOptions = {}) {
    ...
    this.state = undefined as unknown as ModernArtGameState;
  ...
  getHumanPlayerIds(): string[] { return [...this.humanPlayerIds]; }
  ...
  submitBid(amount: number, playerId?: string): boolean {
    if (!this.waitingForHuman) return false;
    if (!this.resolveAction) return false;
    if (playerId && playerId !== this.waitingPlayerId) return false;
  ...
  abort(): void { this.aborted = true; this.resolveAction?.(null as any); }

  getGameInfo(): ModernArtGameInfo {
    ...
    const players: ModernArtPlayerInfo[] = this.state?.players.map(p => ({
      id: p.config.id,
      name: p.config.name,
    ...
    const actions = this.waitingForHuman
      ...
      : [];
    const actionChoices: ModernArtActionChoice[] = actions.map((a, i) => ({
      index: i,
    ...
    // 入札入力が必要か
    const needsBidInput = this.waitingForHuman && this.state?.auctionState != null && (
      this.state.auctionState.auctionType === 'open' ||
    ...
    // オークション状態
    let auctionInfo: ModernArtGameInfo['auctionState'] = null;
      ...
      try {
        ...
        if (this.state.phase === 'play_card') {
          if (isHuman) {
            ...
            if (reasoning) this.addLog(`  💭 ${reasoning}`);
            this.state = executeAction(this.state, action);
  ...
  private async processAuctionStep(): Promise<void> {
    const auction = this.state.auctionState;
    if (!auction) return;
    ...
    if (this.state.phase === 'set_fixed_price') {
      ...
      for (const ev of this.state.lastEvents) this.addLog(`  📦 ${ev}`);
      this.notifyUpdate();
      return;
    }
    ...
    if (auction.auctionType === 'sealed') {
      ...
      for (const p of nonSellers) {
        if (auction.sealedBids[p.config.id] !== undefined) continue;
    ...
    if (this.humanPlayerIds.has(waitingPlayer.config.id)) {
      ...
      this.state = executeAction(this.state, action);
      if (reasoning) this.addLog(`  💭 ${waitingPlayer.config.name}: ${reasoning}`);
      await delay(this.opts.aiDelay / 2);
    }
    for (const ev of this.state.lastEvents) this.addLog(`  📦 ${ev}`);
    this.notifyUpdate();
  ...
  private async waitForHumanAction(playerId: string): Promise<ModernArtAction> {
    this.waitingForHuman = true;
    this.waitingPlayerId = playerId;
    this.notifyUpdate();
    return new Promise<ModernArtAction>((resolve) => {
  ...
  private notifyUpdate() { this.onUpdate?.(); }
}

// ── 表示ヘルパー ──

function describeAction(action: ModernArtAction, state: ModernArtGameState): string {
  switch (action.type) {
    case 'play_card': {
...
function categorizeAction(action: ModernArtAction): ModernArtActionChoice['category'] {
  switch (action.type) {
    case 'play_card':
    case 'play_double':
      return 'play_card';

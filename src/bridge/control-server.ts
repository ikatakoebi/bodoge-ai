...
import { majoStrategyIds, getMajoStrategy, getRandomMajoStrategy } from '../ai/majo-strategies.js';
import type { PlayerConfig, ReplayLog } from '../engine/types.js';
import {
  createGame,
  revealPointCard,
  submitSelection,
...
let inlineCtrl: InlineStepController | null = null;
let playCtrl: PlayController | null = null;
let majoPlayCtrl: MajoPlayController | null = null;
let majoBoardSync: MajoBoardSync | null = null;
let currentMode: 'idle' | 'replay' | 'play' | 'majo' = 'idle';
let logs: string[] = [];
let replayData: ReplayLog | null = null;
let currentRoomId: string | null = null;
/** ボード接続直後のクリーンな状態（リプレイ開始前にリセット用） */
let cleanBoardState: import('./types.js').RemoteGameState | null = null;
...
class InlineStepController {
  ...
  stepBackFn: (() => Promise<boolean>) | null = null;
  ...
  async back(): Promise<boolean> {
    if (this.stepBackFn) return this.stepBackFn();
    return false;
  }
  ...
  setRoundResult(msg: string): void {
  ...
  async delayOrStep(ms: number): Promise<boolean> {
    ...
    if (this._paused) {
      await new Promise<void>((r) => { this._stepResolve = r; });
      if (this._quit) return false;
  ...
  isQuit(): boolean { return this._quit; }
  getPort(): number { return 3216; }
  destroy(): void {}
}
 ...
 */
function generateReplay(playerCount: number, requestedStrategyIds: string[]): ReplayLog {
  // プレイヤー設定
  const assignedStrategies: string[] = requestedStrategyIds.length >= playerCount
    ? requestedStrategyIds.slice(0, playerCount)
    : Array.from({ length: playerCount }, (_, i) =>
        requestedStrategyIds[i] ?? getRandomStrategy().id
      );
...
async function handleSetupBoard(body: string): Promise<object> {
  if (!bridgeClient) return { ok: false, error: '先にルームに接続してください' };

  const params = JSON.parse(body || '{}');
  const sheetUrl: string = params.sheetUrl || '';
  const playerCount: number = Math.min(Math.max(params.playerCount ?? 3, 1), 5);

  // スプレッドシートIDを抽出
  const m = sheetUrl.match(/\/spreadsheets\/d\/([A-Za-z0-9_-]{20,})/);
  if (!m) return { ok: false, error: 'スプレッドシートURLが正しくありません' };
  const sheetId = m[1];
  ...
  try {
    ...
    const data = await res.json() as { ok?: boolean; error?: string; gameState?: Record<string, unknown>; info?: Record<string, unknown>; logs?: string[] };
    ...
    const gameState = data.gameState as Record<string, unknown>;
    ...
    bridgeClient.restoreState(newState as any);
...
async function handleConnect(body: string): Promise<object> {
  const params = JSON.parse(body);
  const boardUrl: string = params.boardUrl || DEFAULT_BOARD_URL;
  const roomId: string | undefined = params.roomId || undefined;
...
async function handleDisconnect(): Promise<object> {
  if (bridgeClient) {
    bridgeClient.disconnect();
    bridgeClient = null;
    currentRoomId = null;
...
async function handleReplayUpload(body: string): Promise<object> {
  try {
    replayData = JSON.parse(body) as ReplayLog;
...
async function handleReplayGenerate(body: string): Promise<object> {
  try {
    const params = JSON.parse(body || '{}');
    const playerCount: number = Math.min(Math.max(params.players ?? 3, 2), 5);
    const requestedStrategies: string[] = params.strategies ?? [];
    const autoStart: boolean = params.autoStart ?? false;
...
async function handleReplayStart(body: string): Promise<object> {
  if (!bridgeClient) return { ok: false, error: '先にルームに接続してください' };
  if (!replayData) return { ok: false, error: 'リプレイデータがありません。先にJSONをアップロードするかAI対戦を生成してください' };
  if (currentMode !== 'idle') return { ok: false, error: `現在 ${currentMode} モードで実行中です` };

  const params = JSON.parse(body || '{}');
  const delay: number = params.delay ?? 2000;
  ...
  const replayCtrl = new ReplayController(replayData, bridgeClient, {
    delay,
    stepController: ctrl as unknown as InstanceType<typeof import('./step-controller.js').StepController>,
  });
...
async function handleReplayPause(): Promise<object> {
  if (!inlineCtrl) return { ok: false, error: 'リプレイが実行されていません' };
  inlineCtrl.pause();
  addLog('一時停止');
  return { ok: true, state: 'paused' };
}

async function handleReplayResume(): Promise<object> {
  if (!inlineCtrl) return { ok: false, error: 'リプレイが実行されていません' };
  inlineCtrl.resume();
  addLog('再生再開');
...
async function handleReplayStep(): Promise<object> {
  if (!inlineCtrl) return { ok: false, error: 'リプレイが実行されていません' };
  inlineCtrl.step();
  addLog('1ステップ進めました');
...
async function handleReplayStop(): Promise<object> {
  if (!inlineCtrl) return { ok: false, error: 'リプレイが実行されていません' };
  inlineCtrl.stop();
...
async function handleReplayBack(): Promise<object> {
  if (!inlineCtrl) return { ok: false, error: 'リプレイが実行されていません' };
...
async function handlePlayStart(body: string): Promise<object> {
  if (!bridgeClient) return { ok: false, error: '先にルームに接続してください' };
  if (currentMode !== 'idle') return { ok: false, error: `現在 ${currentMode} モードで実行中です` };

  const params = JSON.parse(body || '{}');
  const playerCount: number = Math.min(Math.max(params.playerCount ?? 3, 2), 5);
  const requestedStrategies: string[] = params.strategies ?? [];
...
async function handlePlaySelect(body: string): Promise<object> {
  if (!playCtrl) return { ok: false, error: '対戦が開始されていません' };

  const params = JSON.parse(body || '{}');
  const card: number = params.card;
  if (typeof card !== 'number') return { ok: false, error: 'card パラメータが必要です' };
...
function handlePlayState(): object {
  if (!playCtrl) {
...
async function handlePlayStop(): Promise<object> {
  if (!playCtrl) return { ok: false, error: '対戦が開始されていません' };
  playCtrl.abort();
  playCtrl = null;
  currentMode = 'idle';
...
async function handleMajoStart(body: string): Promise<object> {
  if (currentMode !== 'idle') return { ok: false, error: `現在 ${currentMode} モードで実行中です` };
  ...
  // ボード自動接続+セットアップ
  let boardSync: MajoBoardSync | null = null;
  try {
    // 未接続なら自動接続
    ...
    const data = await res.json() as { ok?: boolean; error?: string; gameState?: Record<string, unknown> };
    ...
    addLog(`ボード接続/セットアップスキップ: ${(err as Error).message} — コントロールパネルのみで動作`);
  }

  const ctrl = new MajoPlayController({
    humanPlayerIndices: [params.humanPlayerIndex ?? Math.floor(Math.random() * 4)],
    aiStrategies: requestedStrategies,
...
function handleMajoState(): object {
  if (!majoPlayCtrl) {
    return { ok: true, active: false };
  }
...
async function handleMajoAction(body: string): Promise<object> {
  if (!majoPlayCtrl) return { ok: false, error: '魔女ゲーが開始されていません' };

  const params = JSON.parse(body || '{}');
  const index: number = params.index;
  if (typeof index !== 'number') return { ok: false, error: 'index パラメータが必要です' };
...
async function handleMajoStop(): Promise<object> {
  if (!majoPlayCtrl) return { ok: false, error: '魔女ゲーが開始されていません' };
  majoPlayCtrl.abort();
  majoPlayCtrl = null;
  if (majoBoardSync) {
...
export function startControlServer(port = 3216): void {

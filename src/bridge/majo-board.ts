...
import type { MajoGameInfo } from './majo-play.js';
...
export class MajoBoardSync {
  ...
  private slotCounters: Map<string, string> = new Map();
  /** 管理対象エリア一覧（初期化時に構築） */
  ...
  private updateSlotCounters(info: MajoGameInfo): void {
    const state = this.client.getState();
    const counters = state.counters as Record<string, any>;

    for (const fa of info.fieldActions) {
      const slotId = this.slotCounters.get(fa.id);
      if (!slotId || !counters[slotId]) continue;

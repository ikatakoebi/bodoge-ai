import type { PlayerConfig, GameState } from '../engine/types.js';
import {
  createGame,
  revealPointCard,
  submitSelection,
...
export interface PlayModeOptions {
  /** Number of total players (1 human + rest AI). Default: 3 */
  playerCount?: number;
  /** Strategy IDs for AI players. Random if not specified. */
  aiStrategies?: string[];
  /** Delay in ms between actions. Default: 1500 */
  delay?: number;
  /** Board player slots in order. Slot 0 = human. */
  playerSlots?: string[];
}

export interface PlayGameInfo {
  round: number;
  totalRounds: number;
  pointCard: number | null;
  carryOver: number[];
  humanHand: number[];
  waitingForHuman: boolean;
  finished: boolean;
  finalScores: Array<{ playerId: string; name: string; score: number; rank: number }> | null;
...
const POINT_VALUE_TO_DEF_ID = new Map<number, string>(
  POINT_VALUES.map((v, i) => [v, `P${String(i + 1).padStart(2, '0')}`])
);
const SLOT_TO_LABEL: Record<string, string> = { p0: 'A', p1: 'B', p2: 'C', p3: 'D', p4: 'E' };
...
export class PlayController {
  ...
  private opts: Required<PlayModeOptions>;
  private players: PlayerConfig[] = [];
  /** Map: playerId → board slot */
  private playerSlotMap: Map<string, string> = new Map();

  // Browser-input state
  private _humanHand: number[] = [];
  private _waitingForHuman = false;
  private _humanSelectionResolve: ((card: number) => void) | null = null;
  private _lastBoardState: any = null;
  private _gameState: GameState | null = null;
  private _finished = false;
  private _aborted = false;
  private _finalScores: Array<{ playerId: string; name: string; score: number; rank: number }> | null = null;
  private _currentPointCard: number | null = null;
  private _currentCarryOver: number[] = [];
  private _lastRoundResult = '';
  private _round = 0;
  private _totalRounds = 0;

  constructor(client: BridgeClient, opts: PlayModeOptions = {}) {
    this.client = client;
    this.opts = {
   ...
   */
  setHumanSelection(card: number): boolean {
    if (!this._waitingForHuman) return false;
    if (!this._humanHand.includes(card)) return false;
    if (!this._humanSelectionResolve) return false;
  ...
  private findHandCard(slot: string, value: number): string | null {
  ...
  private findPointCardInDeck(value: number): string | null {
   ...
   */
  private defIdToCardNumber(defId: string): number | null {
   ...
   */
  private waitForHumanCard(validHand: number[]): Promise<number> {
    ...
    const beforeHandIds = new Set(
      this.client.getCardsInArea(handAreaId).map((c) => c.definitionId)
    );

    // ボード状態の差分から動かされたカードを検出するヘルパー
    const detectMovedCard = (incomingState: any): number | null => {
      const currentHandDefIds = new Set<string>();
      for (const inst of Object.values(incomingState.cardInstances) as any[]) {
    ...
    this.client.onBoardAction((incomingState) => {
      if (!this._waitingForHuman) return;
    ...
    return new Promise<number>((resolve) => {
      this._humanSelectionResolve = (card: number) => {
        this.client.onBoardAction(null);
        this.client.onConfirmTurn(null);
        resolve(card);
      };
  ...
  private findRoundCounter(): string | null {
    const state = this.client.getState();
    const counter = Object.values(state.counters).find((c) =>
      c.name.toLowerCase().includes('round') || c.name.toLowerCase().includes('ラウンド')
    );
  ...
  private setupPlayers(): void {
    ...
    const aiStrategies = this.opts.aiStrategies.length >= aiCount
      ...
      : Array.from({ length: aiCount }, (_, i) =>
          this.opts.aiStrategies[i] ?? getRandomStrategy().id
        );
  ...
  async run(): Promise<void> {
    ...
    let gameState: GameState = createGame(this.players);
    ...
    const SLOT_ROW_Y: Record<string, number> = { p0: 90, p1: 0, p2: 72, p3: 18, p4: 54 };
    ...
    while (!isGameOver(gameState)) {
      ...
      if (gameState.currentPointCard === null) break;
      ...
      for (const player of gameState.players) {
        if (player.config.id === HUMAN_ID) continue;

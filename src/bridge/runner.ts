...
import type { ReplayLog } from '../engine/types.js';
...
async function runReplay(args: minimist.ParsedArgs): Promise<void> {
  const url = (args['url'] as string | undefined) ?? DEFAULT_URL;
  const roomId = (args['room'] as string | undefined) ?? 'hagetaka-bridge';
  const playerName = (args['name'] as string | undefined) ?? 'Bridge';
  const delay = (args['delay'] as number | undefined) ?? 2000;
  const logFile = args['log'] as string | undefined;
  ...
  try {
    ...
    replayLog = JSON.parse(raw) as ReplayLog;
  } catch (e) {
    console.error(`エラー: JSONの解析に失敗しました: ${(e as Error).message}`);
    process.exit(1);
  }
...
async function runPlay(args: minimist.ParsedArgs): Promise<void> {
  const url = (args['url'] as string | undefined) ?? DEFAULT_URL;
  const roomId = (args['room'] as string | undefined) ?? 'hagetaka-bridge';
  const playerName = (args['name'] as string | undefined) ?? '人間プレイヤー';
  const delay = (args['delay'] as number | undefined) ?? 1500;
  const playerCount = Math.min(Math.max((args['players'] as number | undefined) ?? 3, 2), 5);
  const strategiesArg = args['strategies'] as string | undefined;
